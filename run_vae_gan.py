...
def arg_parse():
    ...
    parser.add_argument('--ad_loss_weight', type=str,
    ...
    parser.set_defaults(
        ...
        experiment_name="T-VAE-GAN_stock_decreasing_increasing",
...
def train_model(args, dir):
    ...
    vae_gan = models.VAE_GAN(input_dim=num_series_feature, hidden_dim=args.hidden_dim, noise_dim=args.noise_dim,
    ...
    for epoch in range(args.epochs):
        ...
        for real_data_batch, noise_fake_data_batch in zip(real_data_batch_loader, noise_fake):
            ...
            kl_div, _, reconstruction_loss, bce_loss = vae_gan.forward(x=real_data_batch['feature'].to(device), real=real_data_batch['feature'].to(
                device), fake=noise_fake_data_batch['feature'].to(device), alpha=alpha, epoch=epoch, args=args)
            decoder_dis_loss = bce_loss + reconstruction_loss
            encoder_loss = kl_div + reconstruction_loss
