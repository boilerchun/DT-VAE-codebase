...
def arg_parse():
    ...
    parser.set_defaults(
        ...
        experiment_name="DT-VAE_stock_decreasing_increasing",
...
def train_model(args, dir):
    ...
    vae = models.DTVAE(input_dim=num_series_feature, hidden_dim=args.hidden_dim,
    ...
    for epoch in range(args.epochs):
        ...
        for real_data_batch in real_data_batch_loader:
            ...
            kl_div, _, reconstruction_loss = vae.forward(x=real_data_batch['feature'].to(device), real=real_data_batch['feature'].to(
...
def test(model_dir, num_series, log_dir, args):
    ...
    noise_fake = gen_fake_data(
        data_len=num_series, dim=args.noise_dim, seq_len=series_len, batch_size=num_series, diff_load_flag=True)
