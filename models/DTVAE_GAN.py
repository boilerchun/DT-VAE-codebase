...
class Discriminator(nn.Module):
    ...
    def forward(self, x, alpha, grad_reverse):
        if grad_reverse:
...
class DTVAE_GAN(nn.Module):
    ...
    def forward(self, x, real, fake, alpha, first_day, args):
        ...
        _,_,noise_reconstruct = self.decoder.forward(x=torch.concat((fake, first_day), dim=-1))
        reconstruction_loss = self.decoder.reconstruction_loss(mu=mu_decoder.clone(), logvar=logvar_decoder.clone(), real=real.clone())
        
        fake_reconstrcuted_prob,_ = self.discriminator.forward(x=repemetrimized_x, alpha=alpha, grad_reverse=True)
        fake_prob,_ = self.discriminator.forward(x=noise_reconstruct, alpha=alpha, grad_reverse=True)
        real_prob,_ = self.discriminator.forward(x=real, alpha=alpha, grad_reverse=True)

        _,fake_reconstruct_rep = self.discriminator.forward(x=repemetrimized_x, alpha=alpha, grad_reverse=False)
        _,real_rep = self.discriminator.forward(x=real, alpha=alpha, grad_reverse=False)
        dis_reconstruct_loss = torch.mean(torch.sum((fake_reconstruct_rep - real_rep) ** 2, dim=[-1, -2]))

        # bce_loss = self.discriminator.loss(pred=real_prob, label=torch.zeros(real_prob.shape).to(real_prob.device)) 
        # + self.discriminator.loss(pred=fake_prob, label=torch.ones(real_prob.shape).to(real_prob.device)) 
        # + self.discriminator.loss(pred=fake_reconstrcuted_prob, label=torch.ones(real_prob.shape).to(real_prob.device))
        bce_loss = self.discriminator.loss(pred=real_prob, label=torch.zeros(real_prob.shape).to(real_prob.device)) + self.discriminator.loss(pred=fake_prob, label=torch.ones(real_prob.shape).to(real_prob.device)) + self.discriminator.loss(pred=fake_reconstrcuted_prob, label=torch.ones(real_prob.shape).to(real_prob.device))

        return kl_div, repemetrimized_x, reconstruction_loss, dis_reconstruct_loss, bce_loss
